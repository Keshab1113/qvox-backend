...
async function insertLog(logData) {
  try {
    ...
    console.error("[Tracker] Failed to insert log:", err.message);
  }
}
...
router.post("/transcribe", auth, upload.single("file"), async (req, res) => {
  ...
  insertLog(pendingLog).catch((err) =>
    console.error("[Transcribe] Failed to insert pending log:", err.message),
  );
  ...
  try {
    ...
    await pool.execute(
      `UPDATE call_logs SET
       ...
       WHERE request_id = ?`,
      ["failed", httpStatus, errorMessage, durationMs, requestId],
    );
...
async function updateDailyStats(apiKeyId, statDate, status, durationMs) {
  try {
    ...
    console.error("[Stats] Failed to update daily stats:", err.message);
  }
}
